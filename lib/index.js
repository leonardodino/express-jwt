...
module.exports = function(options) {
  ...
  var middleware = function(req, res, next) {
    ...
    async.waterfall([
      async.ensureAsync(function getSecret(callback){
      ...
      async.ensureAsync(function verifyToken(secret, callback) {
        jwt.verify(token, secret, options, function(err, decoded) {
          if (err) {
            return callback(new UnauthorizedError('invalid_token', err));
          } else {
            return callback(null, decoded);
          }
        });
      }),
      async.ensureAsync(function checkRevoked(decoded, callback) {
